...
italy_final$Y[italy_final$Y <= 0] =0.1
...
lasso.cv$lambda.min #0.0002256967 55
...
fit=glmnet(x[,-1],y,family="gaussian",lambda=0.7)
...
p <- colSums(fit$beta!=0) + 1
pred=predict(fit,newx = x[,-1])
#pred=x %*% rbind(fit$a0,fit$beta)
n * log(colSums((y-pred)^2)/length(y)) + n*(log(2*pi)+1)
log(n)*p

fit=glmnet(x[,-1],y,family="gaussian",lambda=1146.95)
tLL <- fit$nulldev - deviance(fit)
k <- fit$df
n <- fit$nobs
bic<-log(n)*k - tLL
...
fit=glmnet(x[,-1],y,family="gaussian",lambda=10.94819)
tLL <- fit$nulldev - deviance(fit)
k <- fit$df
n <- fit$nobs
bic<-log(n)*k - tLL
...
c=cor(data)
findCorrelation(c,cutoff = 0.9, names=TRUE, verbose=TRUE)
...
#ETA, SMARTW_1, TIPOLAU_2, TIPODIP_0, SETT_1, Q1, Q2
